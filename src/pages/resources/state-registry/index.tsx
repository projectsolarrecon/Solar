...
type Entry = {
  ...
  // Optional metadata you may add later to JSON files:
  badges?: {
...
// Small helper: make entries from JSON folder
function entries(): Entry[] {
  return Object.keys(files)
    .map((k) => {
      ...
      return {
        code,
        state: data?.state || code.toUpperCase(),
...
// Basic US regions to power the "Map (beta)" tab.
// Feel free to tweak any assignments later.
...
function CardLink({ e }: { e: Entry }) {
  return (
    ...
    >
      ...
      {/* Optional meta badges (populate later from JSON.badges) */}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="bg-gradient-to-r from-slate-800 to-slate-600 text-white py-10 px-6 rounded-b-3xl shadow-md print:bg-none">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="mt-3 text-slate-200 text-lg max-w-2xl mx-auto">
            SOLAR’s state-by-state analysis of registration requirements, restrictions, and
            compliance steps — with citations and relief-path guidance.
          </p>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-10 print:py-6">
        {/* SEARCH BAR (visible on States + Map) */}
        {(tab === "states" || tab === "map") && (
          ...
          <section className="space-y-8">
            <SectionCard title="How to Use This Hub" icon={<BookOpen className="w-6 h-6 text-indigo-600" />}>
              <ul className="list-disc pl-6 text-slate-700 space-y-1">
                ...
                <li>Each state page follows a standardized format with “At a Glance,” deadlines, restrictions, relief paths, resources, and citations.</li>
              </ul>
            </SectionCard>
            ...
            <SectionCard title="Nationwide Frameworks (What’s Uniform vs. State-by-State)" icon={<Shield className="w-6 h-6 text-emerald-600" />}>
              <ul className="list-disc pl-6 text-slate-700 space-y-1">
                ...
                <li><strong>Megan’s Law:</strong> Public notification requirements exist nationwide but differ in scope and display.</li>
                ...
                <li><strong>Relief paths:</strong> Early termination, re-tiering, or removal petitions are state-specific and often time-bound.</li>
              </ul>
              <p className="mt-3 text-slate-600 text-sm">
                ⚠️ This resource is informational only and not legal advice. Always verify with primary sources and consider consulting counsel.
              </p>
            </SectionCard>

            <SectionCard title="Quick Tips for Compliance" icon={<Shield className="w-6 h-6 text-amber-600" />}>
              <ul className="list-disc pl-6 text-slate-700 space-y-1">
                <li>Know your <strong>initial registration window</strong> on release or arrival in a new jurisdiction.</li>
        ...
        {tab === "map" && (
          <section className="space-y-4">
            <p className="text-slate-600">
              Select a region (top-right) or search a state. A clickable SVG map is planned; this “beta” uses region filters for speed and accessibility.
            </p>
            {filtered.length > 0 ? (
              <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                {filtered.map((e) => <CardLink key={e.code} e={e} />)}
            ...
            <SectionCard title="Frequently Asked Questions" icon={<Shield className="w-6 h-6 text-sky-600" />}>
              <div className="space-y-4 text-slate-700">
                <div>
                  ...
                  <p>This hub is for information only. Laws change quickly; verify all details with primary sources and consider consulting a qualified attorney.</p>
                </div>
                <div>
                  <p className="font-semibold">What if state and local rules conflict?</p>
                  <p>Courts may limit or preempt local ordinances; your state page will note major cases when relevant.</p>
                </div>
                <div>
                  <p className="font-semibold">How current is each state page?</p>
                  <p>Each page shows a “Last reviewed” date. We update as new statutes, rules, or cases appear.</p>
                </div>
                <div>
